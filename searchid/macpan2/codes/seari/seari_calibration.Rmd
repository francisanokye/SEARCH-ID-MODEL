...
```{r}
source("~/Documents/MUN/SEARCH-ID-MODEL/searchid/macpan2/codes/seari/seari_tmb.R")

## -------------------------
## get model searchid from library
## -------------------------

seari_spec = spec

seari_spec = mp_tmb_insert(seari_spec
...
state_labels = c("S", "E", "A", "R", "I","cases")
...
seari_calibrator = mp_tmb_calibrator(
    spec = transformed_spec |> mp_euler() 
  , data = observed_data
  , traj = "cases"  
  , par = c("log_beta_values","gamma_a","mu","gamma_i", "E","A","I","R")
)

mp_optimize(seari_calibrator)
```

```{r}
mp_tmb_coef(seari_calibrator, conf.int = TRUE) 
```

```{r}
euler_coef <- (mp_tmb_coef(seari_calibrator, conf.int=TRUE)
       |> backtrans()
)
...
plot_fit = function(cal_object) {
  ...
  print(fitted_data)
  plot(fitted_data$time, fitted_data$value)
