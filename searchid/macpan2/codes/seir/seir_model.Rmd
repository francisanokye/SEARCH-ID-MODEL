...
source("~/Documents/MUN/SEARCH-ID-MODEL/searchid/macpan2/codes/seir/seir_tmb.R")
seir = spec

## Piecewise Time Variation

beta_changepoints <- c(0, 10, 21, 55, 90)
beta_values <- c(0.779, 0.7463, 0.7203, 0.2612, 0.1021)
time_steps = 170
beta = 0.8 
gamma = 0.2 
state_labels = c("S", "E", "I", "R", "cases")
...
seir_transformed_spec = mp_tmb_insert(
...
seir_transformed_spec_simulator = seir_transformed_spec |> 
                                mp_rk4() |> 
...
(seir_transformed_spec_simulator
  |> mp_trajectory()
...
seir_calibrator = mp_tmb_calibrator(
    spec = seir_transformed_spec |> mp_rk4() 
  , data = observed_data
  , traj = "cases"  
  , par = c("log_beta_values","E","I", "R")
  , default = list(gamma = gamma, beta = beta)
)

mp_optimize(seir_calibrator, type = "nlminb")
```

```{r}
euler_coef <- mp_tmb_coef(seir_calibrator, conf.int=TRUE) |> 
  backtrans()
...
plot_fit = function(cal_object) {
  ...
  ggplot(observed_data, aes(x = dates, y = value, colour = "black"), na.rm = TRUE) +
    geom_line(size = 2) +
    geom_line(aes(x = dates, y = value,colour = "red"), data = fitted_data,  size = 2) +
