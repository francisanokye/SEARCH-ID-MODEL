...
plot_fit = function(cal_object) {
  ...
  p <- ggplot(data = fitted_data, aes(x = dates, y = value)) +
    geom_line(aes(color = factor(matrix)), linewidth = 1.1) +  # Ensure 'matrix' is a valid factor
    geom_line(data = seroprevdata, aes(x = dates, y = value, color = "data"), linewidth = 1.1) +
