...
```{r}
source("~/Documents/MUN/SEARCH-ID-MODEL/searchid/macpan2/codes/searchi/searchi_tmb.R")
```
...
beta_values <- c(0.051, 0.15301, 0.15390, 0.15188, 0.15230) # c(0.010, 0.0357, 0.073, 0.095, 0.095)
...
transformed_spec_simulator = (transformed_spec |> 
 ...
 + geom_line(aes(time, value, colour = state))
  ...
  labs(x = "Date (Dec 2021  - June 2022)", y = "Omicron Incidence", title = "SEARCH-ID Model Incidence Trajectory With macpan2", color = "", size = 5) +

 facet_wrap(~ state, scales = "free")
 + theme_clean()
)

# transformed_spec_simulator$insert$expressions(
#   log_lik ~ dpois(cases, clamp(rbind_time(sim_cases))),
#   .phase = "after"
# )
# transformed_spec_simulator$replace$obj_fn(~ -sum(log_lik))
# 
...
searchi_calibrator = mp_tmb_calibrator(
    spec = transformed_spec |> mp_rk4() 
  , data = observed_data
  , traj = "cases"
  , outputs = "cases"
  , par = c("beta_values",  "phi", "mu", "xi", "theta", "omega", "alpha","E", "A", "I")
)
mp_optimize(searchi_calibrator)
```

```{r}
euler_coef <- (mp_tmb_coef(searchi_calibrator, conf.int=TRUE)
       |> backtrans()
)
